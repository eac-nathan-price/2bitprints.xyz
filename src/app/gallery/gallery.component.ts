...
export class GalleryComponent implements OnInit, OnDestroy {
  ...
  private async initWalls(): Promise<void> {
    ...
    const walls = [
      ...
      // Front wall
      new THREE.Mesh(new THREE.BoxGeometry(wallWidth, wallHeight, wallThickness), visualMaterial),
      // Back wall
      new THREE.Mesh(new THREE.BoxGeometry(wallWidth, wallHeight, wallThickness), visualMaterial),
      // Floor
      new THREE.Mesh(
    ...
    const wallShape = new CANNON.Box(
      new CANNON.Vec3(wallThickness / 2, wallHeight / 2, (wallDepth + 2) / 2)
    );
    ...
    frontWallBody.addShape(
      new CANNON.Box(new CANNON.Vec3(wallWidth / 2, wallHeight / 2, wallThickness / 2))
    );
    ...
    floorBody.addShape(
      new CANNON.Box(new CANNON.Vec3(wallWidth / 2, wallThickness / 2, (wallDepth + 2) / 2))
    );
    ...
    firstStepBody.addShape(
      new CANNON.Box(new CANNON.Vec3(wallWidth / 2, stepHeight, stepWidth / 2))
    );
    ...
    secondStepBody.addShape(
      new CANNON.Box(new CANNON.Vec3(wallWidth / 2, stepHeight / 2, stepWidth / 2))
    );
    ...
    firstStepFrontWallBody.addShape(
      new CANNON.Box(new CANNON.Vec3(wallWidth / 2, stepHeight, stepWallThickness / 2))
    );
    ...
    secondStepFrontWallBody.addShape(
      new CANNON.Box(new CANNON.Vec3(wallWidth / 2, stepHeight / 2, stepWallThickness / 2))
    );
      ...
      if (!this.textureCache.has(image.url)) {
        ...
        loader.load(
          ...
          texture => {
            ...
            console.log(
              `Image loaded: ${image.url} (${timeSinceLastSpawn.toFixed(1)}ms since last spawn)`
            );
            ...
            if (this.isLoading) {
              ...
              console.log(
                `Added to queue, current queue length: ${this.spawnQueue.length} (${timeSinceLastSpawn.toFixed(1)}ms since last spawn)`
              );
        ...
        if (this.isLoading) {
          ...
          console.log(
            `First cached image found, spawning immediately (${timeSinceLastSpawn.toFixed(1)}ms since last spawn)`
          );
          this.isLoading = false;
          this.lastSpawnTime = now;
          this.createCube(image, index).catch(error => {
          ...
          this.spawnQueue.push({ image, spawnIndex: index });
          console.log(
            `Added cached image to queue, current queue length: ${this.spawnQueue.length} (${timeSinceLastSpawn.toFixed(1)}ms since last spawn)`
          );
  ...
  private processSpawnQueue(): void {
    ...
    console.log(
      `Processing spawn for image: ${image.url}, queue length: ${this.spawnQueue.length} (${timeSinceLastSpawn.toFixed(1)}ms since last spawn)`
    );
  ...
  private async updateFilteredCubes(): Promise<void> {
    ...
    const cubesToRemove = this.cubes.filter(
      cube => !filteredImages.some(img => img.url === cube.image.url)
    );
    ...
    const imagesToAdd = filteredImages.filter(
      img => !currentImages.some(current => current.url === img.url)
    );
  ...
  private async createCube(image: PrintImage, spawnIndex: number): Promise<void> {
    ...
    const { wallWidth, wallDepth, wallHeight, totalCubeSize, cubesPerColumn, cubesPerLayer } =
      this.cubeSpacing;
  ...
  private isPositionOccupied(position: THREE.Vector3): boolean {
    ...
    return this.cubes.some(cube => {
      ...
      return (
        Math.abs(cubePos.x - position.x) < safetyMargin &&
  ...
  public respawnCubes(event?: MouseEvent): void {
    ...
    shuffledCubes.forEach((cube, index) => {
      setTimeout(
        ...
        (index * 2000) / shuffledCubes.length
      ); // Stagger over 2 seconds
    });
  }
}
